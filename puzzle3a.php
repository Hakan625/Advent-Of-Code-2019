...
// Loop through the first set of instructions
foreach ($array1 as $value) {
	// Determine what direction the line is to take by looking at the first character of each value (R/L/U/D)
	switch ($value[0]) {
		case 'R':
			// Determine the length that the line is supposed to have
			$instructions = explode('R', $value);
			// Draw a line with the determined specifications
			imageline($img, $varx, $vary, $varx + $instructions[1], $vary, $blue);
			// Shift the variable coordinates by that distance in the correct dimension (x or y)
			$varx += $instructions[1];
			break;		
		// Do the same for all other directions
		case 'L':
			$instructions = explode('L', $value);
			imageline($img, $varx, $vary, $varx - $instructions[1], $vary, $blue);
			$varx -= $instructions[1];
			break;
		case 'U':
			$instructions = explode('U', $value);
			imageline($img, $varx, $vary, $varx, $vary - $instructions[1], $blue);
			$vary -= $instructions[1];
			break;
		case 'D':
			$instructions = explode('D', $value);
			imageline($img, $varx, $vary, $varx, $vary + $instructions[1], $blue);
			$vary += $instructions[1];
			break;
	}
...
// Reset the coordinates back to the previously defined starting point
$varx = 6000;
$vary = 4500;


// In the same way, loop through the second list of instructions
foreach ($array2 as $value) {

	switch ($value[0]) {
		case 'R':
			$instructions = explode('R', $value);
			imageline($img, $varx, $vary, $varx + $instructions[1], $vary, $red);
			$varx += $instructions[1];
			break;
		case 'L':
			$instructions = explode('L', $value);
		...
		case 'U':
			$instructions = explode('U', $value);
			imageline($img, $varx, $vary, $varx, $vary - $instructions[1], $red);
			$vary -= $instructions[1];
			break;
		case 'D':
			$instructions = explode('D', $value);
...
imageellipse($img, 6000, 4500, 606, 606, $blue);
imagepng($img, "demoimage.png");
